...
raw_data<-read.csv('Datasets/Automobile.csv',sep=';', stringsAsFactors = F)

str(raw_data)

raw_data%>%select_if(is.numeric)
...
cols<-names(raw_data)
raw_data[ , (cols) := lapply(.SD,function(x) return(gsub(pattern = '\\?',replacement =  NA,x))), .SDcols = cols]
str(raw_data)
...
data_proc[ , names(data_proc)[sapply(data_proc, is.character)]:=
             lapply(.SD,as.factor),.SDcols = 
             names(data_proc)[sapply(data_proc, is.character)]]
str(data_proc)
